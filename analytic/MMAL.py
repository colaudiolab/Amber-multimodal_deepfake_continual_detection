...
class ACIL(torch.nn.Module):
    def __init__(
        ...
        if isinstance(backbone_output, tuple):
            fusion_output = backbone_output[0]
            video_output = backbone_output[1]
            audio_output = backbone_output[2]
        else:
            fusion_output, video_output, audio_output = backbone_output, backbone_output, backbone_output

        self.buffer = RandomBuffer(fusion_output, buffer_size, **factory_kwargs)
        self.buffer_video = RandomBuffer(video_output, buffer_size, **factory_kwargs)
        self.buffer_audio = RandomBuffer(audio_output, buffer_size, **factory_kwargs)
        self.analytic_linear = linear(buffer_size, gamma, **factory_kwargs)
        self.analytic_linear_video = linear(buffer_size, gamma, **factory_kwargs)
        self.analytic_linear_audio = linear(buffer_size, gamma, **factory_kwargs)
        self.eval()

    @torch.no_grad()
    def feature_expansion(self, video: torch.Tensor, audio: torch.Tensor) -> torch.Tensor:
        outs = self.backbone(video, audio)
        video_out, audio_out, x = outs['video'], outs['audio'], outs['features']
        return self.buffer(x), self.buffer_video(video_out), self.buffer_audio(audio_out)

    @torch.no_grad()
    def forward(self, video: torch.Tensor, audio: torch.Tensor, train=True, phase: int=0) -> torch.Tensor:
        x_out, video_out, audio_out = self.feature_expansion(video, audio)
        fusion_logits = self.analytic_linear(x_out)
        video_logits = self.analytic_linear_video(video_out)
        audio_logits = self.analytic_linear_audio(audio_out)
        outs = {'logits':fusion_logits + video_logits + audio_logits}
        return outs
        # return fusion_logits + video_logits + audio_logits
    ...
    def fit(self, video: torch.Tensor, audio: torch.Tensor, y: torch.Tensor, phase: int = 0, *args, **kwargs) -> None:
        ...
        X, video_out, audio_out = self.feature_expansion(video, audio)
        self.analytic_linear.fit(X, Y)
        residual_Y = Y - self.analytic_linear(X)
        self.analytic_linear_video.fit(video_out, residual_Y)
        self.analytic_linear_audio.fit(audio_out, residual_Y)

    @torch.no_grad()
    def update(self) -> None:
        self.analytic_linear.update()
        self.analytic_linear_video.update()
        self.analytic_linear_audio.update()


class MMALLearner(Learner):
    """
    ...
    def base_training(
        ...
        # # base_training训练好的全连接层没有算入到模型中，导致增量阶段只能重新训练全连接层，而不能利用ACIL的特征增强方法
        # self.backbone = model.module
    ...
    def inference(self, video: torch.Tensor, audio: torch.Tensor) -> torch.Tensor:
        # fusion_logits, video_logits, audio_logits = self.model(video, audio)
        # # final_logits = fusion_logits + video_logits + audio_logits
        # return fusion_logits, video_logits, audio_logits
