...
class MRFA:
    ...
    def feature_augmentation(self, model, video, audio, targets, net_type):
        if net_type == 'GAT':
            ...
            if isinstance(model, torch.nn.DataParallel):
                self.get_feature_augmentation(model.module, model.module.backbone, video, audio, targets, 2, register_forward_prehook_ResNet)
            else:
                self.get_feature_augmentation(model, model.backbone, video, audio, targets, 2, register_forward_prehook_ResNet)
        elif net_type == 'LTI':
            if isinstance(model, torch.nn.DataParallel):
                self.get_feature_augmentation(model.module, model.module.backbone, video, audio, targets, 2, register_forward_prehook_LTI)
            else:
                self.get_feature_augmentation(model, model.backbone, video, audio, targets, 2, register_forward_prehook_LTI)
        else:
    ...
    def register_perturb_forward_prehook(self, model, net_type):
        if net_type == 'GAT':
            ...
            if isinstance(model, torch.nn.DataParallel):
                self.register_perturb_forward_prehook_layers(model.module, model.module.backbone, 42, register_forward_prehook_ResNet)
            else:
                self.register_perturb_forward_prehook_layers(model, model.backbone, 2, register_forward_prehook_ResNet)
        elif net_type == 'LTI':
            if isinstance(model, torch.nn.DataParallel):
                self.register_perturb_forward_prehook_layers(model.module, model.module.backbone, 42, register_forward_prehook_LTI)
            else:
                self.register_perturb_forward_prehook_layers(model, model.backbone, 2, register_forward_prehook_LTI)
        else:
            raise ValueError(f'Unknown net_type {net_type}.')
...
def register_forward_prehook_ResNet(model, backbone, hooks):
    remove_handles = []

    remove_handle_stage_1 = backbone.cross_modal.register_forward_pre_hook(hooks[0])
    remove_handles.append(remove_handle_stage_1)
    remove_handle_stage_2 = backbone.temporal.register_forward_pre_hook(hooks[1])
    remove_handles.append(remove_handle_stage_2)

    return remove_handles

def register_forward_prehook_LTI(model, backbone, hooks):
    remove_handles = []

    remove_handle_stage_1 = backbone.img_emb_layer.register_forward_pre_hook(hooks[0])
    remove_handles.append(remove_handle_stage_1)
    remove_handle_stage_2 = backbone.aud_emb_layer.register_forward_pre_hook(hooks[1])
    remove_handles.append(remove_handle_stage_2)
