...
from .MMTrace import MMTrace
from .Lipsync.lip_sync_model import LipSyncModel
from .OpenAVFF.video_cav_mae import VideoCAVMAEFT
from .ICSAV.class_model import ClassificationModel
from .LTI.model import LTI_Network
...
models: Dict[str, Tuple[
    ...
    "MMTrace": (128, MMTrace, None),
    "LipSyncModel": (128, LipSyncModel, None),
    "VideoCAVMAEFT": (224, VideoCAVMAEFT, None),
    "ICSAV": (128, ClassificationModel, None),
    "LTI": (128, LTI_Network, None)
}
# fmt: on
...
def load_backbone(
    ...
    #     feature_size = 768
        ...
        video_feature = 192
        audio_feature = 64
        # feature_size = (feature_size, video_feature, audio_feature)
    elif isinstance(backbone, MMTrace):
        feature_size = 384
    elif isinstance(backbone, LipSyncModel):
        feature_size = 384
    elif isinstance(backbone, VideoCAVMAEFT):
        feature_size = 1024
    elif isinstance(backbone, ClassificationModel):
        feature_size = 128
    elif isinstance(backbone, LTI_Network):
        feature_size = 40
    else:
